...
const Review = ({ name, relationship, school, review, children }) => {
  return (
    <div class="review card rounded-lg  px-5 pt-5 text-white">
      ...
      <div class="grid grid-cols-12 rounded-2xl bg-black">
        <div class="place-end col-span-10 flex flex-col justify-end rounded-r-xl text-right text-white">
          <span class="pr-3">{name}</span>
          <span class="pr-3">{relationship}</span>
        </div>
        <div class="col-span-2 rounded-xl">{children}</div>
      </div>
    </div>
  );
};
...
const Proof = () => {
  return (
    <section class="proof flex grid-cols-12 flex-col gap-20 gap-y-56 bg-black py-20 px-5 md:grid md:py-36 md:px-20">
